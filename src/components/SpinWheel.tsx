  ...
  return (
    <div className="relative flex flex-col items-center py-10">
      ...
      <div className="relative">
        ...
        >
          <svg viewBox="0 0 100 100" className="w-full h-full transform scale-[1.02]">
            ...
            {/* الدائرة المركزية (The Imperial Vault Core) - رمز Pi النصي المضمون */}
            <g className="filter drop-shadow-[0_0_15px_rgba(251,191,36,0.6)]">
              ...
              <circle cx="50" cy="50" r="12" fill="none" stroke="#fbbf24" strokeWidth="0.2" strokeDasharray="2,2" className="animate-[spin_15s_linear_infinite]" opacity="0.6" />
              
              {/* الرمز π النصي - تم تعديله للوضوح المطلق */}
              <text
                x="50.2"
                y="52.2"
              ...
              <circle cx="47" cy="47" r="5" fill="white" opacity="0.1" />
            </g>
          </svg>
        </motion.div>
        
        {/* نظام الإضاءة LED المحيط */}
      ...
      <motion.div className="mt-12 flex flex-col items-center">
        ...
        {isSpinning ? (
          <motion.div className="flex items-center gap-3" animate={{ opacity: [0.5, 1, 0.5] }} transition={{ repeat: Infinity, duration: 1 }}>
            <Sparkles className="text-gold w-4 h-4" />
            <span className="text-gold font-black tracking-[0.3em] uppercase text-xs">Opening Vault...</span>
            <Sparkles className="text-gold w-4 h-4" />
          </motion.div>
        ) : (
          <p className="text-[10px] text-white/30 uppercase tracking-[0.5em] font-bold">Imperial Security Connection Active</p>
        )}
      </motion.div>
    </div>
  );
