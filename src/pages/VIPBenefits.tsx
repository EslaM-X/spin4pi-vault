              ...
              <CardContent className="p-6">
                    ...
                    <div className={`w-16 h-16 rounded-full bg-gradient-to-br ${tierColors[currentTier.name]?.gradient} flex items-center justify-center shadow-lg`}>
                      {tierIcons[currentTier.name] && (
                        <span className="text-white">
                          {(() => {
                            const Icon = tierIcons[currentTier.name];
                ...
                {nextTier && (
                  <div>
                    ...
                    <Progress 
                      value={((totalSpins - currentTier.min_total_spins) / (nextTier.min_total_spins - currentTier.min_total_spins)) * 100} 
                      className="h-3"
                    />
                  </div>
                )}
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap gap-2 mt-3">
                            {tier.exclusive_rewards.map((reward, i) => (
                              <span 
                                key={i}
                                className={`px-2 py-1 text-xs rounded-full ${colors.bg} ${colors.border} border`}
                              >
                                {reward}
                              </span>
                            ))}
